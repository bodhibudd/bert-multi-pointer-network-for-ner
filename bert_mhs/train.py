...
class Trainer(object):
    ...
    def __init__(self, args, data_loaders, examples, ent_conf):
        ...
        self.model = mhs.NERNet.from_pretrained(args.bert_model, class_num = len(ent_conf))
    ...
    def forward(self, batch, eval=False, answer_dict=None):
        if not eval:
            ...
            loss = self.model(input_ids=input_ids, segment_ids=segment_ids,entity_labels=ent_labels, add_bias=True)
            if self.n_gpu > 1:
                loss = loss.mean()  # mean() to average on multi-gpu.
            loss.backward()
            self.adversarial_train.attack()
            loss_adv = self.model(input_ids=input_ids, segment_ids=segment_ids,entity_labels=ent_labels)
            loss_adv.backward()
        ...
        p_ids, input_ids, segment_ids, all_examples = batch
        eval_file = self.dev_examples
        ent_list = self.model(q_ids=p_ids, input_ids=input_ids, segment_ids=segment_ids, eval_file = all_examples, is_eval=True)
